...
use Core\{Language,ErrorHandler};
...
(empty($viewVars[2])) ? $status_limit = "" : $status_limit = $viewVars[2];

/** Load Paginator **/
$pages = new Paginator(USERS_PAGEINATOR_LIMIT);

$profile = $membersModel->getUserProfile($user);
$main_image = $membersModel->getUserImageMain($profile[0]->userID);

// Set total number of rows for paginator
$total_num_images = $membersModel->getTotalImages($profile[0]->userID);
...
if(!empty($profile)){
  ...
  Core\Extender::load_ext('Profile', 'top');
    ...
    /** Get Users Images **/
    $data['user_images'] = $membersModel->getUserImages($profile[0]->userID, $pages->getLimit($current_page, USERS_PAGEINATOR_LIMIT));

    /** Get User's Groups **/
    $data['user_groups'] = $usersModel->getUserGroupName($profile[0]->userID);
                              ...
                              <?php
                                ...
                                if($isLoggedIn){
                                    ...
                                    if($DispenserModel->checkDispenserEnabled('Messages')){
                                      echo "<tr><td>PM</td><td><a href='".SITE_URL."Messages/New/".$data['profile']->username."' class='btn btn-sm btn-secondary'>".Language::show('members_profile_sendmsg', 'Members')."</a></td></tr>";
                                    }
                                    /* Check to see if Friends Plugin is installed, if it is show link */
                ...
                foreach($data['status_updates'] as $recent){
                  ...
                  echo "<div class='card border-secondary mb-3'>";
                    echo "<div class='card-header'>";
                      echo "<a href='".SITE_URL."Profile/{$recent_userName}'>";
                        echo "<img src=".SITE_URL.IMG_DIR_PROFILE.$recent_userImage." class='rounded' style='height: 25px'>";
                      echo "</a>";
                      echo " <a href='".SITE_URL."Profile/{$recent_userName}'>$recent_userName</a> is feeling $recent->status_feeling..";
                    echo "</div>";
